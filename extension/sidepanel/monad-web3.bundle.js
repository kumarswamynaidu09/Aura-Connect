...
${prettyStateOverride(stateOverride)}`;
  ...
  __export(web3_client_exports, {
    ...
    executeRevokeAccessOnChain: () => executeRevokeAccessOnChain,
  ...
  // src/contracts/AuraConnectABI.ts
  var AuraConnectABI = [
    {
      type: "function",
      name: "createMemory",
  ...
  var MONAD_EXPLORER_URL = "https://testnet.monadscan.com";
    ...
    if (typeof chrome !== "undefined" && chrome.tabs && chrome.scripting) {
      ...
      if (!targetTabId) {
        try {
          ...
          const webTab = allTabs.find((t) => t.id && t.url && (t.url.startsWith("http://") || t.url.startsWith("https://")));
          if (webTab && webTab.id) {
      ...
      try {
        ...
        if (response2) {
          if (response2.error) {
            if (response2.error === "METAMASK_NOT_DETECTED") {
              throw new Error("MetaMask is not detected. Please make sure MetaMask is installed and unlocked in Chrome.");
            }
            const customErr = new Error(response2.error);
            if (response2.code) customErr.code = response2.code;
            throw customErr;
          }
      ...
      if (response.error) {
        ...
        const customErr = new Error(response.error);
  ...
  async function getCurrentChainId() {
    try {
      const hex = await sendProviderRequest({ method: "eth_chainId" });
  ...
  async function executePayForAccessOnChain(memoryId, feeMon = "0.0001") {
    const from16 = await connectUserWallet();
    await ensureMonadNetwork();
    ...
    const hasAccess = await checkHasAccessOnChain(memoryId, from16);
    console.log(`[AURA] Verified on-chain hasAccess: ${hasAccess}`);
    return { hash: txHash, blockNumber: receipt.blockNumber };
  }
  async function executeRevokeAccessOnChain(memoryId, consumerAddress) {
    const from16 = await connectUserWallet();
    await ensureMonadNetwork();
    const calldata = encodeFunctionData({
      abi: AuraConnectABI,
  ...
  return __toCommonJS(web3_client_exports);
})();
